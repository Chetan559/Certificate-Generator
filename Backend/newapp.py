...
def generate_certificates():
    try:
        ...
        # change the column name to match the one in the excel file when eroor occurs
        names = df["Name (As you want to be in the certificate)"].dropna().tolist()
        ...
        with zipfile.ZipFile(zip_buffer, "w", zipfile.ZIP_DEFLATED) as zip_file:
            for name in names:
                ...
                font_style = text_config.get("font_style", "helv")  

                hex_color = HexColor(text_config.get("color", "#000000"))  
